...
import { mediaUpload } from "@/api/create-project/media/media.upload";
import type { UploadedPhoto, PhotoCategory } from "./interfaces";

const UploadPhotosStep = forwardRef<IWizardStepRef, IWizardStepsProps>(
  ({ title, subTitle }, ref) => {
    ...
    // Wizard store
    const { data, setUploadPhotosData } = useWizardStore();

    // Load existing data from store
    useEffect(() => {
      if (data.uploadPhotos) {
        // Note: We can't restore File objects from store, so we'll just restore the metadata
        console.log("Loading existing upload photos data:", data.uploadPhotos);
      }
    }, [data.uploadPhotos]);
    ...
    // Function to upload photos to API
    const handleUploadPhotos = async (): Promise<boolean> => {
      try {
        if (!data.projectId || uploadedPhotos.length === 0) {
          return false;
        }
        ...
        const uploadPromises = uploadedPhotos.map(async (photo) => {
          try {
            ...
            formData.append("description", `Photo: ${photo.name}`);

            const response = await mediaUpload(data.projectId!, formData);
            console.log("Photo uploaded successfully:", response);
            return { success: true, photoId: photo.id, response };
          } catch (error: any) {
        ...
        if (failedUploads.length > 0) {
          ...
          setUploadError(
            `Failed to upload ${failedUploads.length} photos. Error: ${errorDetails}`
          );
          return false;
        }
    ...
    const getStepData = async () => {
      const stepData = {
        ...
        photoUrls: uploadedPhotos.map((photo) => photo.preview),
      };
      // Save to store
      setUploadPhotosData(stepData);

      // Upload photos and check if successful
      const photosUploaded = await handleUploadPhotos();

      // Return both data and success status
      return { data: stepData, success: photosUploaded };
    };
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    Supports JPG, PNG, WebP • Max 10MB per file • Up to 20
                    photos
                  </p>
                </div>
              </div>
          ...
          {isUploading && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <div className="flex items-center gap-2">
                ...
                <p className="text-blue-800 text-sm">Uploading photos...</p>
              </div>
            </div>
          )}
...
UploadPhotosStep.displayName = "UploadPhotosStep";
